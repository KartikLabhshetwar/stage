...
const framePreviews: Record<FrameType, React.ReactNode> = {
  none: (
    <div className="size-full rounded-md border-2 border-dashed border-muted-foreground/50" />
  ),

  solid: <div className="size-full rounded-md border-4 border-primary/80" />,

  glassy: (
    <div className="size-full rounded-md border-2 border-foreground/20 bg-background/10 backdrop-blur-sm" />
  ),
  ...
  dotted: (
    <div className="size-full rounded-md border-2 border-dashed border-primary/80" />
  ),
...
export function BorderControls() {
  ...
  const handleSelect = (value: FrameType) => {
    if (value.startsWith("window-") || value.startsWith("stack-")) {
      ...
      setImageBorder({
        type: type as "window" | "stack",
        theme: theme as "light" | "dark",
        enabled: true,
      });
    } else {
      setImageBorder({
        type: value as Exclude<
          FrameType,
          `${"window" | "stack"}-${"light" | "dark"}`
        >,
        enabled: true,
      });
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-4 border-t border-border pt-4">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setImageBorder({ color: e.target.value, enabled: true })
                  }
        ...
        {["solid", "glassy", "dotted", "eclipse", "ruler", "focus"].includes(
          imageBorder.type,
        ) && (
