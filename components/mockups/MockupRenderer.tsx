...
export function MockupRenderer({
  ...
  if (!definition || !mockup.isVisible) {
    return null;
  }
  ...
  if (!mockupImg) {
    console.error("Failed to load mockup image:", definition.src, mockupStatus);
    return null;
  }
  ...
  const notch = definition.screenArea.notch;
  const notchBottom = notch
    ? (notch.y + notch.height) * mockupHeight
    : screenAreaY;
  const contentStartY = notchBottom;
  const contentHeight = screenAreaHeight - (contentStartY - screenAreaY);
  ...
  if (mockup.imageFit === "cover") {
    if (userImageAspectRatio > contentAspectRatio) {
      ...
      userImageY = contentStartY - (userImageHeight - contentHeight) / 2;
    }
  } else if (mockup.imageFit === "contain") {
    if (userImageAspectRatio > contentAspectRatio) {
      userImageWidth = contentWidth;
      userImageHeight = userImageWidth / userImageAspectRatio;
      userImageX = screenAreaX;
      userImageY = contentStartY + (contentHeight - userImageHeight) / 2;
    } else {
      userImageHeight = contentHeight;
      userImageWidth = userImageHeight * userImageAspectRatio;
      userImageX = screenAreaX + (contentWidth - userImageWidth) / 2;
      userImageY = contentStartY;
    }
  }
