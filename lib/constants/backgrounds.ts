...
export const getBackgroundCSS = (
  ...
  switch (type) {
    case "gradient":
      const gradient =
        gradientColors[value as GradientKey] || gradientColors.sunset_vibes;
      return {
        background: gradient,
        opacity,
      };
      ...
      if (isCloudinaryPublicId) {
        try {
          ...
          if (cloudName) {
            ...
            console.warn(
              `Cloudinary cloud name not found. Background image "${value}" will be used as-is. For optimized images, set NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME in your environment variables.`,
            );
            // Use value as-is, might be a local path or URL
          }
        } catch (error) {
          console.warn(
            `Failed to get Cloudinary URL for background "${value}":`,
            error,
          );
