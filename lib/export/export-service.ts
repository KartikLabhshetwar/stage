...
import { convertStylesToRGB, injectRGBOverrides, generateNoiseTexture } from './export-utils'
 ...
 */
function applyBlurToCanvas(canvas: HTMLCanvasElement, blurAmount: number): HTMLCanvasElement {
  if (blurAmount <= 0) {
    return canvas
  }
        ...
        if (isCloudinaryId) {
          ...
          if (!cloudName) {
            ...
            try {
              ...
              if (
                generatedUrl &&
                (generatedUrl.startsWith('http://') || generatedUrl.startsWith('https://'))
              ) {
...
async function exportBackground(
  ...
  try {
        ...
        try {
          ...
          stylesheets.forEach((sheet) => {
            try {
              if (
                sheet.href &&
                (sheet.href.includes('globals.css') || sheet.href.includes('tailwind'))
              ) {
    ...
    const blurredCanvas =
      backgroundBlur > 0 ? applyBlurToCanvas(canvas, backgroundBlur * scale) : canvas
