...
async function getNoiseTextureFromPreview(): Promise<HTMLCanvasElement | null> {
  ...
  let noiseOverlay = document.getElementById(
...
async function exportOverlays(
  ...
  try {
    ...
    if (!canvasContainer) {
      throw new Error(
        "Canvas container not found. Please ensure the canvas is properly initialized.",
      );
    }

    // Get the actual canvas container dimensions
    // The inner div has explicit width/height set in pixels (canvasW, canvasH)
    const innerCanvasDiv = canvasContainer.querySelector(
    ...
    if (innerCanvasDiv) {
      ...
      if (!actualCanvasWidth || !actualCanvasHeight) {
        ...
        actualCanvasWidth =
          actualCanvasWidth || parseFloat(computedStyle.width) || 0;
        actualCanvasHeight =
          actualCanvasHeight || parseFloat(computedStyle.height) || 0;
      }
    }
    ...
    if (canvasContainer) {
      const textOverlayElements = canvasContainer.querySelectorAll(
        "[data-text-overlay-id]",
      );
      if (textOverlayElements.length > 0) {
        for (const originalElement of Array.from(textOverlayElements)) {
          const overlayId = (originalElement as HTMLElement).getAttribute(
            "data-text-overlay-id",
          );
          ...
          const originalFontSize =
            parseFloat(computedStyle.fontSize) || overlay.fontSize;
          ...
          if (overlay.textShadow?.enabled) {
            ...
            if (shadow && shadow !== "none") {
              clonedElement.style.textShadow = shadow.replace(
                /(\d+\.?\d*)px/g,
                (match, value) => {
    ...
    for (const overlay of imageOverlays) {
      ...
      if (typeof overlay.src === "string" && !overlay.isCustom) {
        ...
        if (isCloudinaryId) {
          ...
          if (!cloudName) {
            console.warn(
              `Cloudinary cloud name not found. Image overlay "${overlay.src}" will be used as-is. For optimized images, set NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME in your environment variables.`,
            );
            ...
            const exportSize = Math.max(
              Math.round(overlay.size * scaleX * 2),
              200,
            );
            ...
            try {
              ...
              console.warn(
                `getCldImageUrl failed for ${overlay.src}, using manual URL construction:`,
                error,
              );
              // Fallback: construct Cloudinary URL manually with 'fit' crop
              imageSrc = `https://res.cloudinary.com/${cloudName}/image/upload/q_auto,f_auto,c_fit,w_${exportSize},h_${exportSize}/${overlay.src}`;
      ...
      await new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(
            new Error(
              `Timeout loading overlay image: ${overlay.src} (URL: ${imageSrc})`,
            ),
          );
        ...
        img.onerror = (error) => {
          ...
          reject(
            new Error(
              `Failed to load overlay image: ${overlay.src} (URL: ${imageSrc}). Error: ${error}`,
            ),
          );
        };
        img.src = imageSrc;
...
async function exportBackground(
  ...
  if (!existingBgElement) {
    throw new Error(
      "Canvas background element not found. Please ensure the canvas is properly initialized.",
    );
  }
  ...
  try {
    ...
    if (backgroundConfig.type === "image" && backgroundConfig.value) {
      await new Promise<void>((resolve) => {
        ...
        if (bgStyle.backgroundImage) {
          const urlMatch = bgStyle.backgroundImage.match(
            /url\(['"]?(.+?)['"]?\)/,
          );
    ...
    const canvas = await html2canvas(container, {
      ...
      ignoreElements: (element) => {
        ...
        return (
          element.id === "export-noise-overlay" ||
          element.id === "canvas-noise-overlay"
        );
      },
      onclone: (clonedDoc, clonedElement) => {
        ...
        const clonedBgElement = clonedElement.querySelector(
          "#export-background-temp",
        ) as HTMLElement;
        if (clonedBgElement) {
          // Ensure filter is preserved in cloned document
          if (backgroundBlur > 0) {
            clonedBgElement.style.setProperty(
              "filter",
    ...
    const canvasWithNoise =
      backgroundNoise > 0
        ? await applyNoiseToCanvas(
            blurredCanvas,
            backgroundNoise / 100,
            width,
            height,
...
export async function exportElement(
  ...
  if (!element) {
    throw new Error(
      "Image render card not found. Please ensure an image is uploaded.",
    );
  }
    ...
    if (perspective3D && imageSrc) {
      ...
      if (has3DTransform) {
        try {
          ...
          const overlayContainer = element.querySelector(
            '[data-3d-overlay="true"]',
          ) as HTMLElement;
          ...
          if (overlayContainer) {
            ...
            if (innerContainer) {
              ...
              const transformedCanvas =
                await capture3DTransformWithModernScreenshot(
                  elementId,
                  options.scale,
                );
              ...
              const scaleX =
                (options.exportWidth * options.scale) / innerRect.width;
              const scaleY =
                (options.exportHeight * options.scale) / innerRect.height;
          ...
          console.warn(
            "Failed to capture 3D transform with modern-screenshot, using Konva image instead:",
            error,
          );
